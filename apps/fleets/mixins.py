...
class FleetScopedMixin(LoginRequiredMixin):
    ...
    Every fleet-scoped view (radios, batteries, configurations) must mix this
    in. It never trusts a submitted `fleet_id`: the fleet always comes from
    the URL, and membership is looked up server-side on every request.
    Non-members get a 404 (not 403) so a fleet's existence isn't confirmed to
    people outside it. Set `require_editor = True` on views that mutate data.
    """
